...
const BodyContent = ({data}) => {
     ...
     return ( 
          <div className="w-full h-full flex flex-col justify-center items-center gap-y-3">
               ...
               <span className="lg:text-4xl md:text-xl sm:text-lg font-semibold text-white"> {data.weather[0].main} & {data.weather[0].description} </span>
               <span className="lg:text-3xl md:text-xl sm:text-lg font-semibold text-white"> Humidity: % {data.main.humidity}</span>
               <span className="lg:text-3xl md:text-xl sm:text-lg font-semibold text-white"> Temp: {Math.floor(data.main.temp/10)} C°</span>
               <span> {data.weather[0].icon === '10d' && (
                    <TiWeatherDownpour size={100}  className='text-neutral-100 opacity-80'/>
               )} </span>
                <span> {data.weather[0].icon === '04d' && (
                    <TiWeatherCloudy size={100}  className='text-white opacity-80'/>
               )} </span>
                <span> {data.weather[0].icon === '03d' && (
                    <TiWeatherPartlySunny size={100}  className='text-white opacity-80'/>
               )} </span>
                <span> {data.weather[0].icon === '11d' && (
                    <IoThunderstormOutline size={100}  className='text-white opacity-80'/>
               )} </span>
               <span> {data.weather[0].icon === '01d' && (
                    <TiWeatherSunny size={100}  className='text-white opacity-80'/>
               )} </span>
               <span> {data.weather[0].icon === '01n' && (
                    <TiWeatherSunny size={100}  className='text-white opacity-80'/>
               )} </span>
                <span> {data.weather[0].icon === '50d' && (
                    <TiWeatherWindyCloudy size={100}  className='text-white opacity-80'/>
               )} </span>
                <span> {data.weather[0].icon === '09d' && (
